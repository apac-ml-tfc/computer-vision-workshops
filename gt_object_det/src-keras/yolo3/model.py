...
from tensorflow.keras.layers import (Add, BatchNormalization, Concatenate, Conv2D, Lambda, LeakyReLU,
...
def make_last_layers(x, num_filters, out_filters, y_name=None):
    ...
    y = compose(
        DarknetConv2D_BN_Leaky(num_filters*2, (3,3)),
        DarknetConv2D(out_filters, (1,1), name=y_name),
    )(x)
    return x, y
...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    x, y1 = make_last_layers(darknet.output, 512, num_anchors*(num_classes+5), y_name="yolo_out_0")
    ...
    x, y2 = make_last_layers(x, 256, num_anchors*(num_classes+5), y_name="yolo_out_1")
    ...
    x, y3 = make_last_layers(x, 128, num_anchors*(num_classes+5), y_name="yolo_out_2")
...
def tiny_yolo_body(inputs, num_anchors, num_classes):
    ...
    y1 = compose(
        DarknetConv2D_BN_Leaky(512, (3,3)),
        DarknetConv2D(num_anchors*(num_classes+5), (1,1), y_name="yolo_out_0"),
    )(x2)
    ...
    y2 = compose(
        Concatenate(),
        DarknetConv2D_BN_Leaky(256, (3,3)),
        DarknetConv2D(num_anchors*(num_classes+5), (1,1), y_name="yolo_out_1"),
    )([x2,x1])
...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    # TODO: Temp K.tile->tf.tile https://github.com/tensorflow/tensorflow/issues/14248
    grid_y = tf.tile(
...
def yolo_output_shape(m, image_shape, anchors, num_classes):
    """
    Parameters
    ----------
    m : int
    ...
    input_shape : array-like
        height, width (both multiples of 32)
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    Returns
    -------
    y_true: list of float32 array, shape like yolo_outputs, xywh are reletive value
    """
